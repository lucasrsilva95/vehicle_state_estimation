...
var_imu_w = 0.25
var_gnss  = 0.01
var_lidar = 1.00   # Part 1
#var_lidar = 110.0   # Part 2
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    delta_pk = delta_x[0:3]
    delta_vk = delta_x[3:6]
    delta_qk = delta_x[6:9]
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    f_ext = C_ns @ imu_f.data[k-1]

    p_check = p_est[k-1] + delta_t*v_est[k-1] + 0.5*(delta_t**2)*(f_ext + g)
    v_check = v_est[k-1] + delta_t*(f_ext + g)
